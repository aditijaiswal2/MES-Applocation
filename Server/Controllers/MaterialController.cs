...
namespace MES.Server.Controllers
{
    ...
    {
        private readonly MaterialRepository _rotorsStyleService;

        public MaterialController(MaterialRepository rotorsStyleService)
        {
        ...
        public async Task<IActionResult> Addworkcenters([FromBody] Materials rotors)
        {
            if (rotors == null)
            {
                return BadRequest();
        ...
        [HttpPut("editwc")]
        public async Task<IActionResult> Updateworkcenters([FromBody] Materials rotors)
        {
        ...
        [HttpGet("getwc")]
        public async Task<ActionResult<IEnumerable<Materials>>> Getworkcenters()
        {
        ...
        [HttpPost("wcQR")]
        public IActionResult GenerateLocQRCode([FromBody] Materials rotors)
        {
            try
            {
                var qrText = $"Material: {rotors.MaterialName}, Material Description: {rotors.Description}, Download-DateTime: {DateTime.Now}";
                var width = 250;
                var height = 250;
                var margin = 0;
